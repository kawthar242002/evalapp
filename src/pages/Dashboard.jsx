...
export default function Dashboard() {
  ...
  const getStudentStats = (session, student) => {
    ...
    const critStats = CRITERES.map((c) => {
      const vals = votes.map((v) => v.notes?.[c.label] ?? 0);
      const avg = vals.reduce((a, b) => a + b, 0) / vals.length;
      return { ...c, avg: avg.toFixed(1) };
    });
    const totalAvg = (votes.reduce((a, b) => a + b.total, 0) / votes.length).toFixed(1);
        ...
        {!stats ? (
          ...
          <>
            ...
            <div style={styles.card}>
              ...
              {stats.votes.map((v, i) => (
                <div key={i} style={styles.voteRow}>
                  ...
                  <span style={styles.voteTotal}>{v.total}/50</span>
            ...
            <div style={styles.card}>
              ...
              {["A — Contenu verbal", "B — Langage non verbal", "C — Support visuel"].map((section, si) => {
                const ranges = [[0, 10], [10, 24], [24, 29]];
                const [start, end] = ranges[si];
                return (
                  <div key={section} style={{ marginBottom: "1.5rem" }}>
                    <p style={styles.sectionLabel}>{section}</p>
                    {stats.critStats.slice(start, end).map((c) => (
            ...
            {(session.students || []).map((s, i) => (
              <div key={i} style={styles.studentChip}>
                <span
                  ...
                  onClick={() => setSelectedStudent({ sessionId: session.id, sessionName: session.name, student: s })}
                >
                  {s}
                </span>
          ...
          <div style={styles.notesSection}>
            ...
            {(session.students || []).map((student) => {
              ...
              return (
                <div
                  key={student}
                  style={{ ...styles.noteRow, cursor: "pointer" }}
